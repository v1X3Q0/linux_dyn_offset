...
int base_inits(kern_img* kernel_local_target)
{
    int result = -1;
    instSet getB;
    ...
    FINISH_IF((kernel_local_target->check_kmap(".head.text", NULL) == 0) &&
        (kernel_local_target->check_kmap(".text", NULL) == 0) &&
        (kernel_local_target->check_kmap(".init.text", NULL) == 0));

    kernel_local_target->insert_section(".head.text", (size_t)binBegin, 0);
    ...
    SAFE_BAIL(kernel_local_target->check_kmap(".head.text", &head_map) == -1);
    head_map->alloc_size = (size_t)text_start - (size_t)binBegin;
    kernel_local_target->insert_section(".text", (size_t)text_start, 0);

    kernel_local_target->insert_section(".init.text", kernel_local_target->kern_sym_fetch("_sinittext"), 0);
